...
import cats._
import react.cats.Mergeable
import react.cats.{Mergeable, Filterable}
...
object ReactiveLibrary {
  ...
  trait Observable[+A] {
    /**
      ...
      *
      * If the method shouldn't be called directly, but only when it first changes its value, use [[react.core.ReactiveLibraryUsage#SignalExtensions#]]
      *
      * == Usage as Event ==
      ...
      * Besides future compositon methods like [[react.core.ReactiveLibraryUsage#FutureExtensions2#executeFuture]] this is
...
trait ReactiveLibrary {
  ...
  /** a time varying value that can be changed (and is generally only changed)
    * when it's [[react.core.ReactiveLibrary.VarTrait#update]] method is called
    */
